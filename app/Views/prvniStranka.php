...
<div class="container mt-4">
    <div class="row g-2">
        <?php if (!empty($articles)): ?>
            <div class="col-lg-6">
                <?php // Main big article ?>
                <?php $main = $articles[0]; ?>
                <div class="ratio ratio-1x1 position-relative overflow-hidden rounded">
                    <div class="position-absolute top-0 start-0 w-100 h-100" style="background-size: cover;background-position:center;background-image: url('<?= base_url('obrazky/sigma/' . esc($main['photo'] ?? '')) ?>')"></div>
                    <div class="position-absolute top-0 start-0 w-100 h-100 bg-dark bg-opacity-50 d-flex flex-column justify-content-center align-items-start text-white text-left p-3">
                        <a class="mt-auto fw-bold fs-2 text-white text-decoration-none" href="<?= site_url('clanek/' . $main['id']) ?>">
                            <?= esc($main['title'] ?? 'Bez názvu') ?>
                        </a>
                        <div class="mt-3 fs-4 text-white"><?= date('j.n.Y', $main['date']) ?></div>
                    </div>
                </div>
            </div>
            <div class="col-lg-6">
                <div class="row g-2">
                    <?php foreach (array_slice($articles, 1, 4) as $article): ?>
                        <div class="col-6">
                            <div class="ratio ratio-1x1 position-relative overflow-hidden rounded">
